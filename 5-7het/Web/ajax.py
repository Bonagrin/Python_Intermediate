...
'''
# Lekérdezés
curl http://localhost:8080/gyumolcs

# Hozzáadás
curl -X POST http://localhost:8080/gyumolcs -H "Content-Type:application/json" -d {\"fajta\":\"alma\",\"darab\": 4}

# Levonás
curl -X POST http://localhost:8080/gyumolcs -d {\"fajta\":\"alma\",\"darab\":-5}
'''

class WebSzerver(http.server.SimpleHTTPRequestHandler):
    gyümölcsök = {'alma': 3, 'banán': 2, 'narancs': 5}
